  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {/* Logo */}
          <Link to="/" style={{ fontFamily: "'Outfit',sans-serif", fontSize: '1.35rem', fontWeight: 600, color: logoColor, letterSpacing: '-.02em', transition: 'color .3s', flexShrink: 0 }}>
        ...
        <ul style={{ display: 'flex', gap: '2.5rem', listStyle: 'none', alignItems: 'center' }} className="nav-desktop">
          {[['Cars', '/cars'], ['How it Works', '/#how']].map(([label, href]) => (
            <li key={label}>
              <Link to={href}
                style={{ fontSize: '.85rem', fontWeight: 400, color: linkColor, transition: 'color .25s', letterSpacing: '.01em', position: 'relative' }}
          ...
          {user ? (
            <>
              <Link to={user.role === 'admin' ? '/admin' : '/dashboard'}
                style={{ fontSize: '.82rem', fontWeight: 500, color: linkColor, transition: 'color .25s', display: 'flex', alignItems: 'center', gap: '.35rem' }}
              ...
              {user.role === 'admin' && (
                <Link to="/admin" style={{ fontSize: '.78rem', fontWeight: 500, color: transparent ? 'rgba(255,255,255,.7)' : '#6b6b69', background: transparent ? 'rgba(255,255,255,.1)' : '#f4f4f2', padding: '.4rem .9rem', borderRadius: 6, transition: 'all .2s' }}
            ...
            <>
              <Link to="/login" style={{ fontSize: '.82rem', fontWeight: 400, color: linkColor, transition: 'color .25s' }}
                onMouseEnter={e => e.target.style.color = linkHover}
                onMouseLeave={e => e.target.style.color = linkColor}
              >Sign In</Link>
              <Link to="/register" style={{ fontSize: '.82rem', fontWeight: 500, background: transparent ? 'rgba(255,255,255,.12)' : '#171715', color: '#fff', padding: '.65rem 1.4rem', borderRadius: 6, border: transparent ? '1px solid rgba(255,255,255,.25)' : 'none', backdropFilter: 'blur(8px)', transition: 'all .25s' }}
                onMouseEnter={e => e.currentTarget.style.background = transparent ? 'rgba(255,255,255,.22)' : '#3a3a38'}
                onMouseLeave={e => e.currentTarget.style.background = transparent ? 'rgba(255,255,255,.12)' : '#171715'}
              >Get Started</Link>
            </>
      ...
      {menuOpen && (
        <div style={{ background: '#fff', borderTop: '1px solid #ebebea', padding: '1.25rem 2rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1.1rem', animation: 'slideDown .2s ease' }}>
          {[['Cars', '/cars'], ['How it Works', '/']].map(([label, href]) => (
            <Link key={label} to={href} style={{ fontSize: '.9rem', color: '#3a3a38', fontWeight: 400 }}>{label}</Link>
          ))}
          <hr style={{ border: 'none', borderTop: '1px solid #ebebea' }} />
          {user ? (
            <>
              <Link to={user.role === 'admin' ? '/admin' : '/dashboard'} style={{ fontSize: '.9rem', color: '#3a3a38' }}>
            ...
            <>
              <Link to="/login" style={{ fontSize: '.9rem', color: '#3a3a38' }}>Sign In</Link>
              <Link to="/register" style={{ fontSize: '.9rem', color: '#171715', fontWeight: 500 }}>Get Started →</Link>
            </>
          )}
        </div>
      )}
